...
struct DXFLIB_EXPORT DL_ArcData {
    ...
    DL_ArcData(double acx, double acy, double acz,
               ...
               double aedx, double aedy, double aedz) {
...
struct DXFLIB_EXPORT DL_PolylineData {
     ...
     */
    DL_PolylineData(
		int pNumber, int pMVerteces, int pNVerteces, int pFlags, double pElevation = 0.0) {
        number = pNumber;
        m = pMVerteces;
        n = pNVerteces;
...
struct DXFLIB_EXPORT DL_VertexData {
     ...
     * Parameters: see member variables.
     */
    DL_VertexData(double pledx, double pledy, double pledz,
...
struct DXFLIB_EXPORT DL_InsertData {
    ...
    DL_InsertData(const std::string& name,
                  ...
                  double iedx, double iedy, double iedz) :
          name(name),
          ipx(ipx), ipy(ipy), ipz(ipz),
          sx(sx), sy(sy), sz(sz),
          angle(angle),
          cols(cols), rows(rows),
          colSp(colSp), rowSp(rowSp), edx(iedx),
          edy(iedy), edz(iedz)
    {
    }
...
struct DXFLIB_EXPORT DL_DimensionData {
    ...
    */
    DL_DimensionData(double dpx, double dpy, double dpz,
        ...
        text(text),
        style(style),
        angle(angle), realValue(realValue),
